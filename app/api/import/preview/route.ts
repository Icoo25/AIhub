...
export async function POST(request:Request){
  try{
    ...
    let image="";
    if(rawImage){try{image=(await assertPublicUrl(new URL(rawImage,response.url).toString())).toString()}catch{image=""}}
    return NextResponse.json({title:meta(html,"og:title")||tag(html,"title")||url.hostname,summary:meta(html,"og:description")||meta(html,"description"),image,url:response.url});
