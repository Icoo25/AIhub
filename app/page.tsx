  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 xl:grid-cols-[1fr_300px]">
      <div className="space-y-6">
        ...
        <div><h2 className="mb-3 flex items-center gap-2 text-sm font-semibold text-[#1b1c16]"><Heart size={15} className="text-[#52621c]"/> Любими инструменти</h2><div className="grid gap-3 sm:grid-cols-2">{tools.filter(t => t.is_favorite).slice(0, 2).map(t => <a href={t.website_url} target="_blank" key={t.id} className="group flex items-center gap-3 rounded-xl bg-[#efeee4] p-4"><ToolIcon name={t.name} websiteUrl={t.website_url} className="h-9 w-9 rounded-lg" imageClassName="h-5 w-5"/><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-semibold text-[#1b1c16]">{t.name}</p><p className="mt-1 truncate text-[8px] text-[#767869]">{t.category}</p><div className="mt-1.5"><ToolRating value={t.rating} compact/></div></div><ArrowRight size={14} className="text-[#767869] transition group-hover:translate-x-0.5"/></a>)}</div></div>
      </div>
