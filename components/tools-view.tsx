...
export function ToolsView() {
  ...
  const filtered = useMemo(() => items.filter(x => (category === "Всички категории" || x.category === category) && (x.name + x.description).toLowerCase().includes(deferredSearch.toLowerCase())), [items, category, deferredSearch]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    catch { setError("Инструментът не беше запазен."); }
    finally { setBusy(false); }
  };
  const remove = async (id: string) => { const tool = items.find(item => item.id === id); if (!(await confirmAction({ title: "Изтриване на AI инструмент", description: `Инструментът „${tool?.name || "Без име"}“ ще бъде премахнат окончателно.`, confirmLabel: "Изтрий инструмента" }))) return; setBusy(true); try { await deleteTool(id); setItems(current => current.filter(item => item.id !== id)); setModal(false); } catch { setError("Инструментът не беше изтрит."); } finally { setBusy(false); } };
  ...
  return <>
    ...
    {filtered.length ? <div className={`grid gap-4 ${view === "grid" ? "sm:grid-cols-2 xl:grid-cols-3" : "grid-cols-1"}`}>{filtered.map(tool => <article key={tool.id} className={`panel group flex flex-col p-5 transition hover:-translate-y-0.5 hover:border-slate-700 hover:shadow-xl ${view === "grid" ? "min-h-60" : "min-h-0"}`}>
      <div className="flex items-start justify-between"><ToolIcon name={tool.name} websiteUrl={tool.website_url}/><div className="flex items-center gap-1"><button title="Любим" onClick={async () => { const next = { ...tool, is_favorite: !tool.is_favorite }; await toggleToolFavorite(tool.id, Boolean(next.is_favorite)); setItems(items.map(x => x.id === tool.id ? next : x)); }} className="p-2 text-slate-600 hover:text-acid"><Star size={16} fill={tool.is_favorite ? "currentColor" : "none"}/></button>{canEdit && <button title="Редактиране" onClick={() => { setEditing(tool); setModal(true); }} className="p-2 text-slate-600 hover:text-white"><MoreHorizontal size={17}/></button>}</div></div>
      ...
      <div className="mt-auto flex items-end justify-between pt-5"><div><p className="mb-2 text-[10px] uppercase tracking-wider text-slate-600">{tool.category}</p><ToolRating value={tool.rating}/></div><div className="flex gap-1">{canEdit && <button title="Изтриване" onClick={() => remove(tool.id)} className="rounded-lg p-2 text-slate-700 opacity-0 transition hover:bg-rose-400/10 hover:text-rose-300 group-hover:opacity-100"><Trash2 size={14}/></button>}<a title="Отвори сайта" href={tool.website_url} target="_blank" className="rounded-lg border border-line p-2 text-slate-500 hover:text-acid"><ExternalLink size={14}/></a></div></div>
    ...
    <Modal open={modal} onClose={() => setModal(false)} title={editing.id ? "Редактиране на AI инструмент" : "Добавяне на AI инструмент"} subtitle={isDemo ? "Промените се пазят само за текущата демо сесия." : "Данните ще бъдат запазени във вашето Supabase пространство."}>
      <form onSubmit={submit} className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs text-slate-400">Име<input required className="field mt-2" value={editing.name || ""} onChange={e => setEditing({...editing, name: e.target.value})}/></label><label className="text-xs text-slate-400">Категория<input required className="field mt-2" value={editing.category || ""} onChange={e => setEditing({...editing, category: e.target.value})}/></label></div><label className="block text-xs text-slate-400">Описание<textarea required rows={3} className="field mt-2 resize-none" value={editing.description || ""} onChange={e => setEditing({...editing, description: e.target.value})}/></label><label className="block text-xs text-slate-400">Адрес на сайта<div className="mt-2 flex items-center gap-3"><ToolIcon name={editing.name || "AI"} websiteUrl={editing.website_url} className="h-11 w-11" imageClassName="h-7 w-7"/><input required type="url" className="field" placeholder="https://..." value={editing.website_url || ""} onChange={e => setEditing({...editing, website_url: e.target.value})}/></div><span className="mt-1.5 block text-[10px] text-slate-500">Иконата се зарежда автоматично от адреса на сайта.</span></label><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs text-slate-400">Статус<select className="field mt-2" value={editing.status} onChange={e => setEditing({...editing, status: e.target.value as ToolStatus})}><option>Активен</option><option>В тестване</option><option>Архивиран</option></select></label><div className="text-xs text-slate-400"><span>Рейтинг</span><div className="mt-2 flex h-[42px] items-center rounded-xl border border-line bg-white/50 px-3"><ToolRating value={editing.rating} onChange={rating => setEditing({...editing, rating})}/></div></div></div><div className="flex justify-between pt-2">{editing.id ? <button type="button" className="text-xs text-rose-300" onClick={() => remove(editing.id!)}>Изтрий инструмента</button> : <span/>}<div className="flex gap-2"><button type="button" onClick={() => setModal(false)} className="btn-secondary">Отказ</button><button disabled={busy} className="btn-primary">{busy ? "Запазване..." : "Запази"}</button></div></div></form>
    </Modal>
  </>;
}
