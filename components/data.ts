...
export async function getTools(): Promise<AITool[]> { if (isDemo) return clone(demoTools); return cached("tools", async () => { const db = createClient()!; const [{ data, error }, favorites] = await Promise.all([db.from("ai_tools").select("id,name,category,description,website_url,status,rating,created_at").order("created_at", { ascending: false }).limit(500), db.from("tool_favorites").select("tool_id")]); if (error) throw error; if (favorites.error) return data; const favoriteIds = new Set((favorites.data || []).map(item => item.tool_id)); return data.map(item => ({ ...item, is_favorite: favoriteIds.has(item.id) })); }); }
export async function getNews(): Promise<AINews[]> { if (isDemo) return clone(demoNews); return cached("news", async () => { const { data, error } = await createClient()!.from("ai_news").select("id,title,summary,source_url,category,published_date,created_at").order("published_date", { ascending: false }).limit(500); if (error) throw error; return data; }); }
