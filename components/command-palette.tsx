...
export function CommandPalette({ open, items, query, onQueryChange, onClose }: { open: boolean; items: CommandPaletteItem[]; query: string; onQueryChange: (value: string) => void; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[150] flex items-start justify-center overflow-y-auto bg-[#1b1c16]/45 p-3 pt-[8vh] backdrop-blur-[4px] sm:p-6 sm:pt-[12vh]" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-label="Командно меню" className="grid max-h-[min(78vh,680px)] w-full max-w-2xl grid-rows-[auto_minmax(0,1fr)_auto] overflow-hidden rounded-2xl border border-[#d7d6ca] bg-[#fffef9] shadow-[0_30px_100px_rgba(27,28,22,.3)]" onMouseDown={event => event.stopPropagation()}>
        <header className="flex min-h-16 items-center gap-3 border-b border-[#e4e3d9] px-4 sm:px-5">
          ...
          <input ref={inputRef} value={query} onChange={event => onQueryChange(event.target.value)} onKeyDown={handleKeyDown} className="min-w-0 flex-1 bg-transparent py-4 text-base text-[#1b1c16] outline-none placeholder:text-[#9a9b8d]" placeholder="Страница, действие или съдържание..." autoComplete="off"/>
          <button type="button" aria-label="Затвори командното меню" onClick={onClose} className="rounded-lg p-2 text-[#767869] hover:bg-[#efeee4]"><X size={18}/></button>
        </header>
        <div className="min-h-0 overflow-y-auto overscroll-contain p-2 sm:p-3">
          <p className="px-3 pb-2 pt-1 text-[10px] font-semibold uppercase tracking-[.12em] text-[#9a9b8d]">Търсене и бързи действия</p>
