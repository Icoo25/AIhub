...
export function ExperimentsView() {
  ...
  return <>
    ...
    {loading ? <div className="panel h-72 animate-pulse"/> : !shown.length ? <div className="panel"><EmptyState title="Няма експерименти" text="Променете филтрите или добавете първия AI тест."/></div> : view === "board" ?
      <div className="panel overflow-x-auto"><div className="flex min-w-max divide-x divide-[#dcd9cd]">{statuses.map(stage => { const stageItems = shown.filter(item => (item.status || "Идея") === stage.name); return <section key={stage.name} className="flex min-h-[520px] w-[310px] shrink-0 flex-col bg-[#f8f7f1]" onDragOver={e => { if (canEdit) e.preventDefault(); }} onDrop={e => { const item = items.find(entry => entry.id === e.dataTransfer.getData("text/plain")); if (item) move(item, stage.name); }}><header className="sticky top-0 z-[1] border-b border-[#dcd9cd] bg-[#f2f1e8]/95 px-4 py-3.5 backdrop-blur"><div className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full shadow-[0_0_0_3px_rgba(255,255,255,.7)]" style={{backgroundColor:stage.color}}/><h2 className="text-xs font-semibold text-[#303128]">{stage.name}</h2><span className="ml-auto grid min-w-6 place-items-center rounded-full bg-white px-2 py-1 text-[9px] font-semibold text-[#67685e] shadow-sm">{stageItems.length}</span></div><div className="mt-2 h-0.5 rounded-full opacity-55" style={{backgroundColor:stage.color}}/></header><div className="flex-1 space-y-3 p-3.5">{stageItems.map(item => <ExperimentCard key={item.id} item={item} canEdit={canEdit} onEdit={() => openEdit(item)}/>)}{!stageItems.length && <div className="grid min-h-28 place-items-center rounded-xl border border-dashed border-[#d3d0c4] bg-white/45 px-4 text-center text-[10px] leading-5 text-[#99978c]">Преместете експеримент в този етап</div>}</div></section>; })}</div></div>
